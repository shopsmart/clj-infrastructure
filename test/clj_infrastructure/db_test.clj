  ...
  (testing "Create table"
    ...
    (jdbc/db-do-commands
     ...
     (jdbc/create-table-ddl
      :test_data
      [[:id (cond
              (= :h2 current-db)       "INTEGER GENERATED BY DEFAULT AS IDENTITY(START WITH 100, INCREMENT BY 1) PRIMARY KEY"
              (= :redshift current-db) "BIGINT IDENTITY(0, 1)"
              :else                    (throw (UnsupportedOperationException. (str "Unsupported database: " current-db))))]
       [:name "VARCHAR(200)"]
       [:sound "VARCHAR(40)"]])))
